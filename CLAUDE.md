...
- **Estado C** (factura sin IRPF + proveedor con `aplica_irpf=true`): aviso amber + botón **"Calcular IRPF"** que entra en **modo preview** (state local `irpfPreview = { base, cuota }` en `PanelDetalleFiscal`). El cálculo NO se persiste hasta confirmación explícita. El bloque cambia visualmente al Estado A con un banner azul claro y los inputs controlados por el state preview (componente `PreviewInputIrpf`). Al hacer **blur fuera del bloque** (detectado con `e.currentTarget.contains(e.relatedTarget)` en el `<div>` contenedor), `guardarPreviewYSalir` valida `cuota <= base + 0.01` y persiste ambos campos en un solo PUT. Si el guardado falla por red/validación, el preview se mantiene para que el usuario reintente sin perder valores. Si navega fuera del panel sin hacer blur, el preview se descarta y al volver se ve Estado C otra vez (BD intacta). Si `cuota > base`, mensaje rojo en el banner azul y bloqueo silencioso del guardado.
- **Estado A — botón "Quitar IRPF"** (sólo visible en Estado A real, no en preview): permite revertir manualmente cuando IRPF se asignó por error. `window.confirm` + `handleGuardarIrpfLocal({ irpf_base: null, irpf_cuota: null })`. Bloqueado si `lote_sage_id != null`.
- **Estado D** (factura sin IRPF + proveedor sin IRPF): bloque oculto.

**Modelo de datos IRPF en factura**: `drive_archivos.irpf_base` e `drive_archivos.irpf_cuota` reflejan SIEMPRE realidad confirmada — o detectada por Gemini en la extracción, o introducida manualmente por el usuario, o calculada y confirmada via el flujo preview/blur. NUNCA cálculos sin confirmar. Esto evita estados huérfanos del tipo "calculé IRPF, después desactivé el proveedor, ahora la factura tiene base/cuota informados pero el proveedor dice que no aplica" (Estado B forzado por error de UX que la versión preview-only previene).
...
**Cherry-pick a `main` con fechas hardcoded:** algunos commits introducen constantes de fecha que delimitan un cambio de comportamiento (ej. `FECHA_FIX_FILTRO_CONTABILIZADAS = '2026-05-04'` en `client/src/pages/Conciliacion.jsx`, usada para mostrar un banner en conciliaciones anteriores al fix). Esa fecha **debe coincidir con el día del cherry-pick a `main`**, no con el día del commit en `dev`. Si el cherry-pick se hace el mismo día que el commit en dev, no hay nada que tocar. Si pasa más de un día (commit en dev hoy, cherry-pick a main mañana o más tarde), **actualizar la constante a la fecha real del cherry-pick antes del push a main**, porque entre medias prod ha podido generar conciliaciones con el código viejo que también deberían marcarse como "antiguas". Patrón observado en commits como `9c0bc51`/`f8475f1` (fecha override SAGE) y el fix de conciliación de Mayor.
