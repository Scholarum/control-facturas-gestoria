...
**Patrón fila expandible para IRPF (proveedores):** los 4 campos IRPF (`aplica_irpf`, `irpf_porcentaje`, `irpf_clave`, `irpf_subcuenta`) son interdependientes: el backend (`parseCamposIrpfBody`) sólo los procesa si `aplica_irpf` viene en el body, y exige los 3 restantes informados cuando `aplica_irpf=true`. La gestión completa (activar/desactivar IRPF + editar los 3 campos) vive en un panel expandible de la fila de la tabla — `client/src/components/proveedores/PanelIrpfProveedor.jsx`. Cada `<tr>` lleva un chevron al inicio que despliega un segundo `<tr>` con el panel; sólo una fila puede estar expandida a la vez (estado `filaExpandidaId` en `Proveedores.jsx`). El modal de "+ Nuevo proveedor" (`ModalProveedor.jsx`) **NO** gestiona IRPF — el flujo es crear el proveedor → expandir su fila en la tabla → marcar IRPF. Razón: para activar IRPF en proveedores ya existentes hace falta un disparador desde la tabla, así que centralizamos la gestión ahí y evitamos duplicar JSX y validación en 2 sitios.

Las 2 celdas inline (`IRPF %` y `Cta. IRPF`) en la fila siguen funcionando como atajo para editar valores cuando `aplica_irpf=true` — el helper `FilaProveedorEditable.guardarCampoIrpf` envía el set IRPF completo (los 4 campos del proveedor + el override del campo concreto) por la regla del backend. Si `aplica_irpf=false`, las celdas se ven deshabilitadas (`—` con tooltip "Marca 'Aplica IRPF' en el panel del proveedor para activar"); la única forma de activar IRPF es expandir la fila.
